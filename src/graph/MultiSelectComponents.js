                        ...
                        <MultiSelect
                            ...
                            onChange={(selected) => {
                                if (selected.length === 0) {
                                    // Cleared - set both to empty/null
                                    setHypercare([]);
                                    setHypercareStatus(null);
                                } else if (selected.length > 1) {
                                    // Multiple selected - keep only last one
                                    const lastValue = selected[selected.length - 1];
                                    setHypercare([lastValue]);
                                    setHypercareStatus(lastValue.value);
                                } else {
                                    // Single selection
                                    setHypercare(selected);
                                    setHypercareStatus(selected[0].value);
                                }
                            }}
                            labelledBy="Select"
                            className="multiselect-input"
